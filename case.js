...
◉ Nama: ${pushname}
◉ Nomer: ${senderNumber}
◉ Serial: ${db.data.users[sender].serial}
◉ Verified: ${calender}
◉ Saldo: Rp 1000
◉ Grade: Newbie
◉ Rank : 
...
conn.sendButDoc(from, teks,copyright,null, butprofile, options1)
...
if(isCmd && !user && body !== "register"){
let teks =`Kamu belom terdaftar di dalam database
silakan registrasi terlebih dahulu agar bisa menggunakan fitur bot
tekan tombol register di bawah ini untuk memulai registrasi`
let mok = [{"buttonId": `register`,"buttonText": {"displayText": `REGISTER`},"type": "RESPONSE"}]
return conn.sendButLoc(from, teks,copyright,fs.readFileSync('./stik/thumb.jpeg'), mok)
}   
...
//Bot tidak bisa di akses di pc kecuali premium
 ...
 //User 
const userLevel = user? db.data.users[sender].level : false
const userExp = user? db.data.users[sender].exp : false
const userId = user? db.data.users[sender].id : false
const amountExp = Math.floor(Math.random() * 10) + 50
const requiredExp = 1000 * userLevel
const userPersen = userExp/requiredExp*100
const userVerified = user? db.data.users[sender].date : false
...
conn.sendMessage(from, { id: m.key.id, contextInfo: {   forwardingScore: 10, isForwarded: true, externalAdReply:{showAdAttribution: true, title: `${botName}`,body:`Runtime ${runtime(process.uptime())} `,previewType:"PHOTO",thumbnail: fs.readFileSync('./stik/reply.jpg'),mediaUrl: "https://bit.ly/SatganzDevs",sourceUrl: "https://bit.ly/SatganzDevs"}},showAdAttribution: true, text: teks }, { quoted: dev })
...
let ahpus = await conn.copyNForward(mess.key.remoteJid, msg).catch(e => console.log(e, msg))
await conn.sendMessage(from, { id: m.key.id, contextInfo: {   forwardingScore: 10, isForwarded: true, externalAdReply:{showAdAttribution: true, title: `${baileysVersion}`,body:`Runtime ${runtime(process.uptime())} `,previewType:"PHOTO",thumbnail: fs.readFileSync('./stik/reply.jpg'),mediaUrl: "https://bit.ly/SatganzDevs",sourceUrl: "https://bit.ly/SatganzDevs"}},showAdAttribution: true, text: `*]───── ANTI DELETE ─────[*
...
break 
    ...
    var quotes = await conn.sendMessage(from, { id: m.key.id, contextInfo: { externalAdReply:{ showAdAttribution: true, title: `${botName}`,body:`Runtime ${runtime(process.uptime())} `,previewType:"PHOTO",thumbnail: fs.readFileSync('./stik/reply.jpg'),mediaUrl: "https://bit.ly/SatganzDevs",sourceUrl: "https://bit.ly/SatganzDevs"}},showAdAttribution: true, text: data.result.quotes })
    await sleep(300)
	...
	*SOUNDCLOUD DOWNLOADER*
	...
	conn.sendButImage(from, teks, `© Cyber Bot`, fs.readFileSync('./stik/bot.jpg'), mok, {quoted: dev})             
	}
	break
...
reply(`*TIKTOK DOWNLOADER*\n\n*Author:* ${res.result.username}\n\nVideo sedang dikirim...`)
conn.sendMessage(from, {video: {url: res.result.video}, caption: 'Nih!'}, {quoted:dev})
      ...
      case "dm": {
        ...
        if (!room) {
         setReply(`\`\`\`Kamu Sedang Tidak Berada Di Sesi Menfess\`\`\``)
          ...
          if (other)
            await conn.sendMessage(other, { id: m.key.id, contextInfo: {   forwardingScore: 10, isForwarded: true, externalAdReply:{showAdAttribution: true, title: `${baileysVersion}`,body:`Runtime ${runtime(process.uptime())} `,previewType:"PHOTO",thumbnail: fs.readFileSync('./stik/reply.jpg'),mediaUrl: "https://bit.ly/SatganzDevs",sourceUrl: "https://bit.ly/SatganzDevs"}},showAdAttribution: true, text:`\`\`\`Partner Telah Meninggalkan Sesi Menfess\`\`\``}, { quoted: dev })
      ...
      case "confirm":
        {
          ...
          if (room) {
            await  conn.sendMessage(room.a, { id: m.key.id, contextInfo: {   forwardingScore: 10, isForwarded: true, externalAdReply:{showAdAttribution: true, title: `${baileysVersion}`,body:`Runtime ${runtime(process.uptime())} `,previewType:"PHOTO",thumbnail: fs.readFileSync('./stik/reply.jpg'),mediaUrl: "https://bit.ly/SatganzDevs",sourceUrl: "https://bit.ly/SatganzDevs"}},showAdAttribution: true, text:`\`\`\`BERHASIL MENYAMBUNGKAN MENFESS\`\`\``}, { quoted: dev })
            room.state = "CHATTING"
          } else if (!room) {
            let buttmess = {
...
let listSerch = []
 ...
 // Log(yaya)
  ...
  if(module.includes("@adiwajshing/baileys")){
    listSerch.push({
    title: module,
    rowId: `${prefix}$ npm i ${module}@https://github.com/officialdittaz/BaileysFix`,
...
let listmodule = Object.keys(lala.dependencies)
    ...
    case "verif":
      ...
      case "atk":
        ...
        if (!terern)
          return reply(`Examples of use : ${command} *>Target<*|*>Amount<*`);
